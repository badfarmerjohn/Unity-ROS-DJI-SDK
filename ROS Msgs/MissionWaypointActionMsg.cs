    ...
    namespace dji_msgs
    {
        public class MissionWaypointActionMsg : ROSBridgeMsg
        {
            ...
            public override string ToYAMLString()
            {
                ...
                sb.AppendFormat("\"action_repeat\": {0}, ", _action_repeat);
                sb.AppendFormat("\"command_list\": [{0}], ", string.Join(", ", _command_list));
                sb.AppendFormat("\"command_parameter\": [{0}]", string.Join(", ", _command_parameter));
                sb.Append("}");
                return sb.ToString();
            }
        }
